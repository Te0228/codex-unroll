...
| # | 功能 |
|---|---|
| ~~F14~~ | ~~工具调用配对~~ ✅ **已做**，见 §6.9.1 |
| ~~F15~~ | ~~轮次分组 + 整轮折叠~~ ✅ **已做**：分组由 §6.8 的图承担，折叠见 §6.9.2 |
| ~~F16~~ | ~~耗时可视化~~ ✅ **已做**，见 §6.9.3 |
| ~~F17~~ | ~~Token 用量图表~~ ✅ **已做**，见 §6.9.4。★ 画的是**单步增量**不是累计，理由见下 |
| ~~F18~~ | ~~复制（脱敏后）~~ ✅ **已做**，见 §6.9.5 |
| ~~F19~~ | ~~`function_call.arguments` 的 JSON 字符串二次解析~~ ✅ **已提前到 v0.1**，见 §6.7 |
| ~~F20~~ | ~~大内容按 markdown 标题分段~~ ✅ **已做**，见 §6.9.6 |
...
| Turn 头折叠钮 | §6.9.2，折起后只剩头尾并说清藏了几条 |
| Step 内耗时条 | §6.9.3，刻度是**会话级**共享的，这样才能横向比较哪次工具慢 |
| 全图之后的用量图 | §6.9.4。放在所有 Turn **之后**而不是顶上——顶上会变成 §6.0/F5 明确否掉的摘要卡片区 |
...
### 6.9 v0.2 量化视图与详情面板增强（SPEC §5 P1 全部落地）

> ⚠️ **编号撞车提醒**：§5 的功能编号 F14/F20 与 §14.4 的验收编号 F14/F20 是**两套命名空间**
> （§14.4 的 F14 是「六组计数」、F20 是「对比度」）。这是历史遗留，两边都已被引用，
> 强行改号的风险大于收益。引用时务必带上章节号。

#### 6.9.1 工具调用配对（F14）

`call_id` 把「发起调用」和「拿到结果」串起来，详情面板内互跳。两条工具路径的
`payload.type` 不同，但 `call_id` 的位置一样：
...
★ **三种「配不上」都必须活下来**，且 UI 要把跳转按钮**藏起来**而不是显示一个点了没反应的：

| 情况 | 什么时候出现 |
|---|---|
| 只有调用没有结果 | 被中断、或正在跟随还没写回来——**rollout 里最常见的半截状态，不是坏数据** |
| 只有结果没有调用 | 从中间截断的文件 |
| 同一个 `call_id` 出现多次 | 不该发生；真发生了取第一条，其余进 `extras`，**绝不静默丢弃**（§3.4） |

#### 6.9.2 整轮折叠（F15）

轮次分组本身已由 §6.8 的图承担（Turn 就是一层）。这里只加折叠：折起来只剩 Turn 头尾，
正文全收，并说清藏了几条。**默认展开**——§6.8.8「一条都不能少」仍然成立，折叠必须由用户主动按。

#### 6.9.3 工具耗时条（F16）

耗时 = 结果条目的时间戳 − 调用条目的时间戳，靠 `call_id` 配对。
实测夹具 04：`exec_command` 1800ms，`apply_patch` 500ms。

> ⚠️ **算不出耗时时必须是「不知道」，不是 0。** 三种情况：只有调用没有结果、
> 任一侧缺时间戳、时间戳解析不了。画成 0 宽的条会被读成「瞬间完成」——那是编造数据。
> 一条都算不出时整块**不画**。

#### 6.9.4 Token 用量图（F17）

> ## ★★ `total_token_usage` 是**会话累计值**，不是单步用量
>
> 夹具 04 的八条 `token_count` 依次是：
> `11840 → 24800 → 38720 → 54020 → 70760 → 88780 → 108260 → 128620`
>
> 单调递增，因为字段名就叫 **total**。直接把它画成曲线，会得到一条永远向上的斜线，
> 看着像每步都在暴涨，其实只是累计量——**图会骗人**。
>
> 所以默认画**单步增量**（做差），累计值另给一个切换。第一个点的增量等于它本身。

**增量可能为负，不许夹到 0。** 机制已在 Codex 源码核实：
`TokenUsageInfo::append_last_usage`（`protocol/src/protocol.rs:2122`）走 `add_assign`，
正常路径确实单调递增；但 `fill_to_context_window`（同文件 2127）会**整体替换**该结构——
只填 `total_tokens`，把 `input_tokens` / `output_tokens` 归零。于是后一条读数可以低于前一条。

> 早先这里写的原因是「会话被 compact 压缩后计数回落」，**那是错的**：
> compact 另有 `active_context_tokens` 走分析埋点，不碰 `total_token_usage`。
> 已按源码更正。抹掉负值等于把「这里发生过一次重置」的信号删掉。

#### 6.9.5 复制（F18）

复制的是**已脱敏**的 `preview` / `rawPretty`（脱敏在归一化层就做完了，§9.1）。
旁边常显一句说明，让用户知道复制出去的是遮蔽过的。

> ⚠️ 生产是 `file://`，**不是 secure context**，`navigator.clipboard` 未必可用。
> 做法是「优先 `navigator.clipboard.writeText`，失败回退隐藏 textarea + `execCommand`」，
> 两条路都要能跑，失败要显式报错而不是静默。
> **不为此增加 IPC 方法**——preload 严格只暴露 8 个（验收 E7）。

#### 6.9.6 大内容分段（F20，原 Q7）

正文超长时（AGENTS.md 注入实测 8846 字符）按 markdown 标题（`^#{1,6} `）切成可折叠的段。
**没有标题就退回原来的「截断 + 展开全部」，不硬切**——把一段连续散文从中间劈开比不分段更难读。

#### 6.9.7 下钻层数不变

§6.9.2 的整轮折叠、§6.9.6 的分段折叠都属于**本层内部导航**，不计入下钻层数
（口径同 §14.8），因此不打 `data-drill`。全应用真正的下钻仍然恰好两层。
...
| 阶段 | 内容 | 完成标准（见 §14） | 状态 |
|---|---|---|---|
| **M0** | 脚手架 | Electron Forge + Vite + TS | ✅ |
| **M0.5** | 测试夹具 | 4 份脱敏夹具 + 实测期望值 | ✅ |
| **M1** | 升 TS、接入 React、空窗口 | `npm start` 出窗口；`webPreferences` 满足 §9 | ✅ |
| **M2** | `shared/rollout.ts` + 单测 | §14.2 的 A/B/C/D 共 40 条全绿，`src/shared/` 覆盖率 ≥90% | ✅ |
| **M3** | 主进程 IPC | §14.3 的 E1–E7 + 跟随 G1–G7 | ✅ |
| **M4** | React UI | §14.4 的 F1–F24 | ✅ |
| **M5** | 实时跟随 | §14.5 的 G1–G7 | ✅ |
| **M6** | 打磨 + README + 截图 | §14.6 端到端冒烟已脚本化 | ✅ |
| **M7** | 打包分发 | `npm run make` 产物可在干净机器打开 | ✅ zip 产物可跑 |
| **v0.2** | 主区「图」视图 Session ▸ Turn ▸ Step | §6.8 / §14.9 | ✅ |
| **v0.2** | 中英双语 | §15 | ✅ |
| **v0.2** | §5 P1 全部（F14–F18 / F20） | §6.9 / §14.10 | ✅ |

> §14.6 里仍需人工的两步（真跑一次 `codex exec` 验证跟随的真实滚动、
> 在干净的 macOS 用户下打开打包产物）不在自动化范围内，每次发版前手动过一遍。
...
| ~~Q7~~ | ~~超大内容要不要按 markdown 标题分段折叠？~~ | ✅ **已做**，见 §6.9.6。没有标题的内容退回原来的「截断 + 展开全部」 |
...
#### 14.8.1 v0.2 期间的两条实测校正

**① `file://` 是 secure context，`navigator.clipboard` 真的能用（F18）。**
原本按「`file://` 不是安全上下文所以剪贴板不可用」设计，实测推翻了：
Electron 43 + `sandbox: true` + `loadFile()` 下 `window.isSecureContext === true`，
`clipboard-write` 权限为 `granted`，**窗口有焦点时 `writeText` 真的写进系统剪贴板**。

但回退路径不是装饰：**窗口失焦时 `writeText` 抛 `NotAllowedError: Document is not focused`**，
而带真实点击手势的 `execCommand('copy')` 即使失焦也能成功。
「从别的应用切回来、直接点复制」正是失焦场景，所以两条路都必须留。

两个连带的坑：
- `await navigator.clipboard?.writeText(t)` 是**陷阱**：没有 `clipboard` 对象时可选链得到
  `undefined`，`await undefined` 直接 resolve，于是「报告成功、其实什么都没复制」。必须显式判存在。
- 回退用的 textarea 只能移到屏幕外，**不能 `display:none` / `visibility:hidden`**——
  那样的元素选不中，复制出来是空串。

**② F20 的分段阈值是「≥2 段」而不是「≥1 段」。**
被当作 F20 典型输入的那条 8 846 字符正文（`03-edge-cases` 索引 7）**只有一个** markdown 标题，
切出来是「一整段」——多一次点击、零导航价值，还会撞坏 §14.4 F12 的既有断言。
真正的 F20 载荷是 `01` 索引 3（23 041 字符 → 29 段）。单段一律退回「截断 + 展开全部」。
...
**G 组（40 条）· `src/renderer/components/StepGraph.test.tsx`** —— 图视图组件
...
| G15–G15f | F15 整轮折叠：折叠后正文清空、Turn 骨架仍在、说清藏了几条（夹具 01 = **17 条**；夹具 04 = 20 / 28 / 7）。★ 条数从**全量**条目算，不随过滤变化 |
| G16–G16e | F16 耗时条：夹具 01 `apply_patch` = **161ms**；夹具 04 七条宽度 `100 / 100 / 27.78 / 100 / 27.78 / 100 / 100 %`——**27.78% = 500/1800，证明刻度是会话级而不是每个 Step 各算各的**。★ 算不出耗时时给文案不给 0 宽条；`maxDuration` 为 undefined 时整块不画而工具行仍在 |
| G17–G17f | F17 用量图：默认单步增量（图上搜不到累计值 `24800`/`128620`），切累计后才出现；末 Step 无 `token_count` 显示 `—` 不是 0；★ 负增量 `-12000` 照实渲染、`data-sign="neg"`、不夹到 0；G17f：全部过滤掉**且**三轮全折叠，9 根柱子仍在 |
| G18 / G18b | `metrics.css` 回归：不许出现无 `text-overflow` 配套的 `overflow: hidden`（会裁掉干线与接线）；`.metric-fill` 宽度必须读 `var(--w, 0%)` |

另有 §14.6 冒烟里的 **19 条**图视图与量化视图断言——`offsetHeight` / `scrollWidth` 这类只有真实渲染器能测。

### 14.10 v0.2 验收 · 配对与量化视图（§5 P1）

纯函数两组，共 **28 条**，是 F14/F16/F17 的地基（同 §14.2 的纪律：不全绿不动 UI）。

**P 组（11 条）· `src/shared/pairing.test.ts`** —— `call_id` 配对

| 编号 | 断言 |
|---|---|
| P1–P4 | 夹具 01 配成 1 对（索引 11 ⇄ 12）；夹具 04 的 7 对全配上且按首次出现排序；两条工具路径都认；`counterpart` 双向可达 |
| P5 | 非工具条目没有对家 |
| **P6** | ★ 三种「配不上」各一条：只有调用 / 只有结果 / 压根没有 `call_id`。都返回 `undefined`，UI 据此**藏起**按钮 |
| P7 | ★ `call_id` 重复时取第一条，其余进 `extras` —— **绝不静默丢弃**（§3.4） |
| P8–P9 | 非工具条目蹭到同一个 `call_id` 也没有对家；空输入不崩 |

**M 组（17 条）· `src/shared/metrics.test.ts`** —— 耗时与 token

| 编号 | 断言 |
|---|---|
| M1–M2 | 夹具 04 的 7 次调用全算得出；实测 `exec_command` 1800ms / `apply_patch` 500ms |
| **M3** | ★ 算不出耗时时是 `undefined` **不是 0**（4 种成因各一条 + `maxDuration` 为 undefined 时整块不画）。0 宽的条会被读成「瞬间完成」，那是编造 |
| M4 / M4b / M5 | 只有结果没有调用则跳过；工具名取不到时是空串不是 `'undefined'`；`timeMs` 缺失/非法一律 `undefined` |
| **M6–M7** | ★ 累计值原样保留 + 做差得单步增量。夹具 04 实测 `11840→24800→…→128620`，增量 `11840, 12960, …, 20360` |
| M8 | 每个点标出属于哪个 Turn 的第几个 Step |
| **M9** | ★ 累计读数回落时负增量**照实保留不夹到 0**；纵轴上限取绝对值 |
| M10–M12 | 没有 `token_count` 时序列仍在但无数值（上层不画）；空图不崩；夹具 01 与 §14.2 C11 对得上 |

**详情面板三组（36 条）· `src/renderer/components/DetailPanel.test.tsx`**

| 组 | 覆盖 |
|---|---|
| P1–P9 | F14 互跳：配对成功双向可达；★ **配不上时按钮不渲染**，不是渲染一个点了没反应的 |
| K1–K11 | F18 复制：两条剪贴板路径各自可用、失败显式报错、复制内容是脱敏后的、隐藏 textarea 必须可选中（`display:none` 会复制出空串） |
| N1–N15 | F20 分段：`01` 索引 3 的 23 041 字符 → **29 段**，索引 2 的 10 467 字符 → 3 段（首段是标题前的引子）；无标题或只有 1 段时退回截断 |

图视图三组（G15–G18b，19 条）在 §14.9。
