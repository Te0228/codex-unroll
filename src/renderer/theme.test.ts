...
describe.each([
  ...
  it.each(GROUPS.map((g) => [g.label, g.cssVar] as const))('%s（%s）达标', (_label, cssVar) => {
...
describe('F21 · 分类不只靠颜色', () => {
  ...
  it('(符号, 组名) 组合唯一——灰度下每组仍可辨识', () => {
    const pairs = GROUPS.map((g) => `${g.symbol}${g.label}`);
    expect(new Set(pairs).size).toBe(GROUPS.length);
  });
